...
def create_map_for_d_star(env):
	# print("Terrain Pose Dict")
	# print(env.mdp_generator_fn().terrain_pos_dict)
	# print("Terrain Matrix")
	# print(env.mdp_generator_fn().terrain_mtx)
	...
	# print("COUNTER LOCATIONS", env.mdp_generator_fn().terrain_pos_dict['X'])
	# print("ACCESSIBLE COUNTER LOCATIONS", env.mdp_generator_fn().get_accessible_counter_locations())
	dstar = Dstar(m)
	...
	# ox, oy = [], []
	# for x, y in env.mdp_generator_fn().terrain_pos_dict['X']:
	# 	ox.append(x)
	# 	oy.append(y)

	# path_planning_algo = Dijkstra(ox, oy, grid_size, robot_radius)


	return m, dstar, obstacle_list
	# return path_planning_algo, obstacle_list
...
def create_action_dict(path_xy_coordinates, high_level_action):
	...
	if "pick" in high_level_action or "put" in high_level_action or "place" in high_level_action:
...
for i in range(1000):
	...
	# joint_action = (action_dict[0], action_dict[])
	# next_state, timestep_sparse_reward, done, info = env.step(joint_action)	
